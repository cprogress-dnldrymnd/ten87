(function ( $ ) {
	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder, returnBreakpoints ) {
			...
			if ( $holder.hasClass( 'qodef--pinterest-slider' ) ) {
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'obsius_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodefAppear = {
		init: function () {
			this.holder = $('.qodef--has-appear:not(.qodef--has-horizontal-appear), .qodef--has-custom-appear');
...
(function ($) {
	...
	var qodefWooModifiedButton = {
		init: function () {
			var $button = $( '.single_add_to_cart_button, .button.add_to_cart_button,a.added_to_cart.wc-forward, .products a.button' );
			var $svg = '<svg class="qodef-svg--button-arrow" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="9px" height="9px" viewBox="0 0 9 9" style="enable-background:new 0 0 9 9; fill:currentColor;" xml:space="preserve">' +
				'<path d="M8,9V1.8L0.9,8.8L0.2,8.1L7.3,1H0V0h9v9H8z"></path>' +
				'<path d="M8,9V1.8L0.9,8.8L0.2,8.1L7.3,1H0V0h9v9H8z"></path>' +
				'</svg>';
			qodefWooModifiedButton.addHtml( $button, $svg );
		},

		addHtml: function ( button, svg ) {
			if ( button.length ) {
				button.each( function () {
					var thisButton = $( this );

					if ( ! thisButton.find( '.qodef-svg--button-arrow' ).length ) {
						thisButton.wrapInner( '<div class="qodef-m-text"></div>' );
						thisButton.append( svg );
					}
				} );
			}
		}
	...
	/*
	 *  Add html to price filter button
	 */
	var qodefPriceFilterButton = {
		init: function () {
			var $button = $( '.woocommerce.widget_price_filter .button' );
			var $svg = '<svg class="qodef-svg--button-arrow" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="9px" height="9px" viewBox="0 0 9 9" style="enable-background:new 0 0 9 9; fill:currentColor;" xml:space="preserve">' +
				'<path d="M8,9V1.8L0.9,8.8L0.2,8.1L7.3,1H0V0h9v9H8z"></path>' +
				'<path d="M8,9V1.8L0.9,8.8L0.2,8.1L7.3,1H0V0h9v9H8z"></path>' +
				'</svg>';
			qodefPriceFilterButton.addHtml( $button, $svg );
		},
